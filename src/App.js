    ...
    const experiments = [
        [{
                ...
                content: {
                    ...
                    prerequisites: "- Fabric environment set up in Ubuntu (WSL2), Docker running, fabric-samples repository cloned, Go and Docker installed correctly.",
                    system_design: "Asset: Car Record\nFields: make, model, color, owner\nFunctions: InitLedger, CreateCar, QueryAllCars, ChangeCarOwner",
...
node query.js # Queries all cars on ledger`,
                    result: "Students have gained hands-on experience with the basic life cycle of assets on the blockchain using Hyperledger Fabric.",
                },
                outputs: {
...
Transaction completed successfully at 2024-08-10T18:35:42.123Z`
...

            {
                ...
                content: {
                    ...
                    prerequisites: "- Ubuntu 20.04 or WSL2 setup, Docker & Docker Compose, Node.js v14.x or Go v1.17+, Fabric binaries and Docker images (version 2.2+ recommended).",
                    system_design: "Asset: Book Asset\nFields: BookID, Title, Author, ISBN, Status (Available/Issued), BorrowerID\nUser Asset Fields: UserID, Name, Email, Role (Student/Faculty)",
...
- Query Book: \`peer chaincode query -C mychannel -n librarycc -c '{"function":"queryBook","Args":["B002"]}'\``,
                    result: "This experiment demonstrated how Hyperledger Fabric can be used to implement a decentralized Library Management System with asset lifecycle, smart contracts, and blockchain-based transparency.",
                },
                outputs: {
...
Borrower field cleared.`
            ...
            // Add similar outputs for other experiments...
            {
                ...
                content: {
                    ...
                    prerequisites: "- Ubuntu 20.04 / WSL2, Docker & Docker Compose, Node.js v14.x / Go, Hyperledger Fabric Samples, Binaries, and Docker Images.",
                    system_design: "Asset: Student Record\nFields: studentID, name, department, dob, email, grades (Object with subject-grade mapping)",
...
- List All Students: \`peer chaincode query -C mychannel -n studentcc -c '{"function":"getAllStudents","Args":[]}'\``,
                    result: "This lab demonstrated how blockchain can enhance security, transparency, and traceability in managing sensitive academic data like student records using Hyperledger Fabric.",
                },
                outputs: {
...
"Chemistry": "B+"
            ...
            // Continue with other experiments following the same pattern...
            {
                ...
                content: {
                    ...
                    prerequisites: "- Docker & Docker Compose, Node.js (v14.x) or Go (if using Go SDK), Hyperledger Fabric samples, binaries, and Docker images.",
                    system_design: "Asset: User Wallet\nFields: userID, name, balance (Number - Wallet balance in currency units)",
...
- List All Users: \`peer chaincode query -C mychannel -n walletcc -c '{"function":"getAllUsers","Args":[]}'\``,
                    result: "This lab demonstrated a secure P2P fund transfer system using Hyperledger Fabric, showcasing blockchain's ability to track transactions immutably, prevent double-spending, and remove intermediaries.",
                },
                outputs: {
...
"transactionCount": 3
            ...
            // Add remaining experiments with similar output structure...
            {
...
"status": "ACTIVE"
    ...
    // Effect to handle initial welcome message - now instant
    useEffect(() => {
    ...
    const handleCommand = (e) => {
        ...
        if (e.key === "Enter") {
            ...
            setHistory((prev) => [
                ...
                {
                    type: "command",
                    text: "user@labmanual:~$ " + trimmedCommand
                },
            ]);
            ...
            switch (cmd) {
                case "help":
                    ...
                    newHistory.push({
                        type: "info",
                        text: " - list: List all available experiments",
                    });
                    newHistory.push({
                        type: "info",
                        text: " - view <experiment_name>: View details for a specific experiment (use the command name)",
                    });
                    newHistory.push({
                        type: "info",
                        text: " - output: Show available programs with sample outputs",
                    });
                    newHistory.push({
                        type: "info",
                        text: " - output <program_name>: Show sample output for a specific program",
                    });
                    newHistory.push({
                        type: "info",
                    ...
                    newHistory.push({
                        type: "info",
                        text: " - fullscreen: Toggle fullscreen mode (or press F11)",
                    });
                    break;
                ...
                case "list":
                    newHistory.push({
                        type: "info",
                        text: "Available experiments (use the command name to view details):",
                    });
                ...
                case "view":
                    const exp = experiments.find((e) => e.command === arg);
                    if (exp) {
                        setCurrentView(exp.content);
                        newHistory.push({
                            type: "info",
                            text: `Viewing details for: ${exp.title}`,
                        });
                        newHistory.push({
                            type: "info",
                            text: '--- Use "clear" to return to the main prompt ---',
                        });
                    } else {
                        newHistory.push({
                            type: "error",
                            text: `Error: Experiment '${arg}' not found. Type 'list' to see all experiments.`,
                        });
                    }
                    break;
                ...
                case "output":
                    if (!arg) {
                        // Show list of programs with available outputs
                        newHistory.push({
                            type: "info",
                            text: "Available programs with sample outputs:",
                        });
                        experiments[0].forEach((exp) => {
                            if (exp.outputs) {
                        ...
                        newHistory.push({
                            type: "info",
                            text: 'Usage: output <program_name> (e.g., "output fabcar")',
                        });
                    } else {
                        // Show specific program outputs
                        const targetExp = experiments[0].find((e) => e.command === arg);
                        if (targetExp && targetExp.outputs) {
                            newHistory.push({
                                type: "info",
                                text: `Sample outputs for ${targetExp.title}:`,
                            });
                            newHistory.push({
                                type: "info",
                                text: "=".repeat(60),
                            ...
                            Object.entries(targetExp.outputs).forEach(([key, output]) => {
                                newHistory.push({
                                    type: "info",
                            ...
                            newHistory.push({
                                type: "error",
                                text: `Error: No sample outputs available for '${arg}'. Type 'output' to see available programs.`,
                            });
                        }
                    }
                    break;
                ...
                default:
                    newHistory.push({
                        type: "error",
                        text: `Error: Command '${trimmedCommand}' not recognized. Type 'help' to see available commands.`,
                    });
                    break;
            }
    ...
    return ( <
        div className = {
            `terminal-container ${isFullscreen ? "fullscreen" : ""}`
        }
        onClick = {
            handleTerminalClick
        } >
